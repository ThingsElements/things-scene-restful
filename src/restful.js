...
export default class Restful extends ValueHolder(RectPath(Shape)) {
  ...
  get withCredentials() {
    return !!this.model.withCredentials;
  }

  set withCredentials(withCredentials) {
    this.model.withCredentials = withCredentials;
  }
  ...
  _makeRequest(url) {
    ...
    this.httpRequest.onreadystatechange = this.spreadData.bind(this);
  ...
  spreadData() {
    if (this.httpRequest.readyState === 4) {
      if (this.httpRequest.status === 200) {
        var data = JSON.parse(this.httpRequest.responseText)
...
scene.Component.register('restful', Restful);
